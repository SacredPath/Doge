...
function initializeErrorElements() {
  ...
  tierError = document.getElementById('tierError');
  paymentError = document.getElementById('paymentError');
  
  // Debugging
  console.log('Error elements initialized:', { fullNameError, emailError, tierError, paymentError });
  
  // Check if all error elements exist
  if (!fullNameError || !emailError || !tierError || !paymentError) {
...
// Debugging
console.log('Form elements:', { submitBtn, fullName, email, investmentAmount, paymentMethod });
...
if (!investmentAmount) {
  console.error('Investment amount select not found');
}
if (!paymentMethod) {
